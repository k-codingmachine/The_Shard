...
//      return "redirect:/item/insertQna?pageNum=1"; //여기에 itemNum을 info에서 가지고 넘겨야함.
   ...
   // wishlist
   @GetMapping("/wishlist")
   @PreAuthorize("isAuthenticated()") // 회원전용
   @ResponseBody
   public ResponseEntity<String> wishList(@RequestParam("itemNum") int itemNum, @RequestParam("email") String email) {
      ...
      int result = itemService.wishListSelect(itemNum, email); // SQL문을 통해 위시리스트에 상품이 있나 체크

      if (result == 0) { // 위시리스트에 이미 담겨있다
   ...
   // wishlist 체크
   @GetMapping(value = "/checkWishlist", produces = MediaType.APPLICATION_JSON_VALUE) // json타입으로 변경해서 데이터를 보내준다.
   @ResponseBody
   public ResponseEntity<Integer> checkWishlist(@RequestParam("itemNum") int itemNum, @RequestParam("email") String email) {
	   
	   int result = itemService.wishListSelect(itemNum, email); // SQL문을 통해 위시리스트에 상품이 있나 체크
	   
	   return new ResponseEntity<Integer>(result, HttpStatus.OK); // 결과를 json타입으로 보내준다.
	  }
